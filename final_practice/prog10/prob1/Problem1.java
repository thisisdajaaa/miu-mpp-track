package final_practice.prog10.prob1;
...
public class Problem1 {
    ...
    public static List<Customer> specialCustomers(List<Account> accts) {
        return accts.stream()
                .filter(x -> x.getCustomer().getAddress().getCity().length() == x.getCustomer().getCheckingAccount().getId().length())
                .map(Account::getCustomer)
                .sorted(Comparator.comparing(Customer::getCustId, Comparator.reverseOrder()))
                .toList();
        //implement
    ...
    public static List<Customer> specialAccounts(List<Customer> custs) {
        return custs.stream().filter(x -> x.getCheckingAccount().getBalance() < 100 && x.getCheckingAccount().getBalance() > 50)
                .filter(x -> x.getFirstName().startsWith("A")).sorted(Comparator.comparing(Customer::getLastName)).toList();
        //implement
    }
