package final_practice.prog2.prob1;
import java.util.*;
...
public class Problem1 {
	...
	public static List<Customer> specialCustomers(List<Account> accts) {
		return accts.stream()
				.filter(x -> x.getId().length() == x.getCustomer().getAddress().getCity().length())
	...
	public static List<Customer> specialAccounts(List<Customer> custs) {
		return custs.stream()
				.filter(x -> x.getCheckingAccount().getBalance() < 100)
				.filter(x -> x.getCheckingAccount().getBalance() > 50)
				.filter(x -> x.getFirstName().toUpperCase().startsWith("A"))
